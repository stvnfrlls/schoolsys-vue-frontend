    ...
    <div v-else>
      ...
      <div
        ...
        <div class="card-header bg-white d-flex align-items-center justify-content-between py-3">
          ...
          <span class="text-muted small">{{ item.sections.length }} section(s)</span>
        </div>
            ...
            <div class="d-flex align-items-center gap-3 mb-3">
              ...
              <div class="d-flex gap-1">
                <button
                  ...
                  :key="q"
                  class="btn btn-sm"
                  :class="activeQuarter === q ? 'btn-primary' : 'btn-outline-secondary'"
                  @click="activeQuarter = q; loadGrades()">
                  Q{{ q }}
                </button>
              </div>
            </div>

            <div v-if="gradeLoading" class="text-center py-4 text-muted">Loading...</div>
            <div v-else-if="gradeError" class="text-center py-4 text-danger">{{ gradeError }}</div>

            <div v-else>
              <div v-if="gradeComponents.length === 0" class="alert alert-warning small">
                No active grading components found for this subject.
                ...
                <table class="table table-bordered table-hover small mb-0">
                  ...
                  <tbody>
                    <tr
                      ...
                      <td class="text-nowrap">{{ fullName(row.enrollment.student) }}</td>
                      <td
                        v-for="comp in gradeComponents"
                        :key="comp.id"
                        class="text-center"
                        ...
                        <input
                          ...
                          :disabled="savingCell === cellKey(row.enrollment.id, comp.id)" />
                      </td>
                      <td class="text-center fw-semibold">{{ row.finalGrade ?? '—' }}</td>
                    ...
                    <tr v-if="gradeSheet.length === 0">
                      <td :colspan="gradeComponents.length + 3" class="text-center text-muted py-4">
              ...
              <div class="mt-2 small text-muted">
                Scores save automatically. Final grade computed by server. Failing threshold: below 75.
              </div>
            </div>
          </div>
          <div class="modal-footer">
            <button class="btn btn-secondary btn-sm" @click="closeGradeModal">Close</button>
          </div>
        </div>
      </div>
    </div>
...
import { gradingComponentService, studentGradeService } from '@/services/grading'
...
async function loadGrades() {
  ...
  try {
    ...
    // Load active enrollments for this section
    ...
    // Load grades for all enrollments
...
async function onScoreChange(row, component, event) {
  ...
  try {
    ...
    existingGrades.value = [
      ...existingGrades.value.filter((g) => g.enrollment_id !== row.enrollment.id),
      ...res.data,
    ]
  } catch (err) {
